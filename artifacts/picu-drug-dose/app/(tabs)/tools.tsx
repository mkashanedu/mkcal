...
// Tables 3 & 4 — SBP & DBP percentiles by age (1-17), gender, height percentile column
// Height columns 0-6 → 5th, 10th, 25th, 50th, 75th, 90th, 95th height percentile for age
// Each age entry: [p50vals, p90vals, p95vals, p99vals] — each array has 7 height-col values
type BPRow = [number[], number[], number[], number[]]; // [p50, p90, p95, p99]
...
function getBPBand(val: number, p50: number, p90: number, p95: number): number {
  if (val < p50) return 0; // Normotensive / Hypotensive
  if (val < p90) return 1; // High-Normal
  if (val < p95) return 2; // Pre-hypertensive
  return 3;                // Hypertensive
}
...
function classifyBP(
  ageYears: number, gender: "M" | "F", heightCm: number, sbp: number, dbp: number
): BPCalcResult | null {
  ...
  return {
    col, htLabel: HT_PCT_LABELS[col] ?? "50th",
    sbp50, sbp90, sbp95, sbp99,
    dbp50, dbp90, dbp95, dbp99,
    sbpBand, dbpBand, overallBand,
    classification: b.label, color: b.color,
  };
}
